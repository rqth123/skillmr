...
class MRAgent:
    def __init__(self, mode='O', exposure=None, outcome=None, AI_key=None, model='MR', num=100, bidirectional=False,
        ...
        self.agent_config = locals() # 确保你的 config 能被存下来
        # 如果你原来没有存 config，可以手动接收一个 kwargs 或者保证从 run_pipeline 传过来的 agent_config 可以被访问。
        # ⚠️ 简单的做法：因为你在 run_pipeline2.py 是这样传参的 MRAgent(**config)
        # 所以你可以直接用 self 访问，我们在下面的方法中使用 getattr 兼容：

    # ==========================================
    # 🧬 轨道 B：本地遗传学流程 (Genetic Track)
    # ==========================================
    def gen_step0(self):
        # 计算 GRM
        genetic_dir = getattr(self, 'genetic_dir', '/home/h1c011/lyjjj/genetic')
        run_genetic_script('00_make_grm.sh', genetic_dir)

    def gen_step1(self):
        # 运行 FastGWA
        genetic_dir = getattr(self, 'genetic_dir', '/home/h1c011/lyjjj/genetic')
        run_genetic_script('01_run_fastgwa.sh', genetic_dir)

    def gen_step2(self):
        # 运行 GREML 计算全局遗传度
        genetic_dir = getattr(self, 'genetic_dir', '/home/h1c011/lyjjj/genetic')
        run_genetic_script('02_run_greml.sh', genetic_dir)

    def gen_step3(self):
        # 计算 Local H2
        genetic_dir = getattr(self, 'genetic_dir', '/home/h1c011/lyjjj/genetic')
        run_genetic_script('03_run_local_h2.sh', genetic_dir)

    def gen_step4(self):
        # 运行共定位分析
        genetic_dir = getattr(self, 'genetic_dir', '/home/h1c011/lyjjj/genetic')
        run_genetic_script('04_run_coloc.sh', genetic_dir)
    # 定义path并创建文件夹
    ...
    def step5_get_gwas_id(self, keyword):
        ...
        if not json_list or len(json_list) == 0:
            print(f"⚠️ 找不到 '{keyword}' 的 GWAS 数据，跳过 LLM 筛选。")
            return "[]"
    ...
    def run(self, step=None):
        # Check if we're resuming from a previous run - validate existing files
        if step is None:
            step = [1, 2, 3, 4, 5, 6, 7, 8, 9]

        # Pre-run validation: check which steps are already completed
        completed_steps = []
        for s in [1, 2, 3, 4, 5, 6, 7, 8, 9]:
            if s == 1 and os.path.exists(os.path.join(self.path, 'Exposure_and_Outcome.csv')):
                if self._validate_csv(os.path.join(self.path, 'Exposure_and_Outcome.csv')):
                    completed_steps.append(s)
            elif s == 2 and os.path.exists(os.path.join(self.path, 'Exposure_and_Outcome.csv')):
                if self._validate_csv(os.path.join(self.path, 'Exposure_and_Outcome.csv')):
                    completed_steps.append(s)
            elif s == 3 and os.path.exists(os.path.join(self.path, 'Outcome_SNP.csv')):
                if self._validate_csv(os.path.join(self.path, 'Outcome_SNP.csv')):
                    completed_steps.append(s)
            elif s == 4 and os.path.exists(os.path.join(self.path, 'Outcome_SNP.csv')):
                if self._validate_csv(os.path.join(self.path, 'Outcome_SNP.csv')):
                    completed_steps.append(s)
            elif s == 5 and os.path.exists(os.path.join(self.path, 'Outcome_SNP.csv')):
                if self._validate_csv(os.path.join(self.path, 'Outcome_SNP.csv')):
                    completed_steps.append(s)
            elif s == 6 and os.path.exists(os.path.join(self.path, 'Exposure_and_Outcome.csv')):
                if self._validate_csv(os.path.join(self.path, 'Exposure_and_Outcome.csv')):
                    completed_steps.append(s)
            elif s == 7 and os.path.exists(os.path.join(self.path, 'Exposure_and_Outcome.csv')):
                if self._validate_csv(os.path.join(self.path, 'Exposure_and_Outcome.csv')):
                    completed_steps.append(s)
            elif s == 8 and os.path.exists(os.path.join(self.path, 'mr_run.csv')):
                if self._validate_csv(os.path.join(self.path, 'mr_run.csv')):
                    completed_steps.append(s)
            elif s == 9 and os.path.exists(os.path.join(self.path, 'mr_run.csv')):
                if self._validate_csv(os.path.join(self.path, 'mr_run.csv')):
                    completed_steps.append(s)

        if completed_steps:
            print(f"[RESUME] Found {len(completed_steps)} already completed steps: {completed_steps}")

        # 判断是否需要进行step1
        if 1 in step:
            if 1 in completed_steps:
                print(f"[SKIP] step1 already completed and validated")
            else:
                print('#########step1#########')
                self.step1()
                if not self._validate_csv(os.path.join(self.path, 'Exposure_and_Outcome.csv')):
                    raise RuntimeError("step1 output validation failed. Check the CSV file and try again.")
        if 2 in step:
            if 2 in completed_steps:
                print(f"[SKIP] step2 already completed and validated")
            else:
                print('#########step2#########')
                self.step2()
                if not self._validate_csv(os.path.join(self.path, 'Exposure_and_Outcome.csv')):
                    raise RuntimeError("step2 output validation failed. Check the CSV file and try again.")
        if 3 in step:
            if 3 in completed_steps:
                print(f"[SKIP] step3 already completed and validated")
            else:
                print('#########step3#########')
                self.step3()
                if not self._validate_csv(os.path.join(self.path, 'Outcome_SNP.csv')):
                    raise RuntimeError("step3 output validation failed. Check the CSV file and try again.")
        if 4 in step:
            if 4 in completed_steps:
                print(f"[SKIP] step4 already completed and validated")
            else:
                print('#########step4#########')
                self.step4()
                if not self._validate_csv(os.path.join(self.path, 'Outcome_SNP.csv')):
                    raise RuntimeError("step4 output validation failed. Check the CSV file and try again.")
        if 5 in step:
            if 5 in completed_steps:
                print(f"[SKIP] step5 already completed and validated")
            else:
                print('#########step5#########')
                self.step5()
                if not self._validate_csv(os.path.join(self.path, 'Outcome_SNP.csv')):
                    raise RuntimeError("step5 output validation failed. Check the CSV file and try again.")
        if 6 in step:
            if 6 in completed_steps:
                print(f"[SKIP] step6 already completed and validated")
            else:
                print('#########step6#########')
                self.step6()
                if not self._validate_csv(os.path.join(self.path, 'Exposure_and_Outcome.csv')):
                    raise RuntimeError("step6 output validation failed. Check the CSV file and try again.")
        if 7 in step:
            if 7 in completed_steps:
                print(f"[SKIP] step7 already completed and validated")
            else:
                print('#########step7#########')
                self.step7()
                if not self._validate_csv(os.path.join(self.path, 'Exposure_and_Outcome.csv')):
                    raise RuntimeError("step7 output validation failed. Check the CSV file and try again.")
        if 8 in step:
            if 8 in completed_steps:
                print(f"[SKIP] step8 already completed and validated")
            else:
                print('#########step8#########')
                self.step8()
                if not self._validate_csv(os.path.join(self.path, 'mr_run.csv')):
                    raise RuntimeError("step8 output validation failed. Check the CSV file and try again.")
        if 9 in step:
            if 9 in completed_steps:
                print(f"[SKIP] step9 already completed and validated")
            else:
                print('#########step9#########')
                self.step9()
    ...
    def _validate_csv(self, csv_path):
        ...
        if not os.path.exists(csv_path):
            print(f"[VALIDATION] File not found: {csv_path}")
            return False
        if os.path.getsize(csv_path) == 0:
            print(f"[VALIDATION] File is empty: {csv_path}")
            return False
        # Check that it has at least a header and one data row
        try:
            df = pd.read_csv(csv_path, nrows=5)
            if len(df) == 0:
                print(f"[VALIDATION] File has no data rows: {csv_path}")
                return False
        except Exception as e:
            print(f"[VALIDATION] Failed to read CSV: {csv_path}, error: {e}")
            return False
        return True
